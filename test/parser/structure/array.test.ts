...
describe('Array parser', () => {
    it.each(['string', 'number'] as const)(
        ...
        (type) => {
            ...
            const parseElement = (element: unknown) =>
                parse(element)
                    ...
                    .elseLazyGet(() =>
                        parse(element)
                            .asString()
                            .elseThrow('elemenet is neither string nor number')
                    );
            ...
            const readonlyParser = parse(array).asReadonlyArray(parseElement);

            expect(mutableParser.elseThrow(message)).toStrictEqual(array);
            expect(mutableParser.elseThrow(errorMessgage)).toStrictEqual(array);
    ...
    it.each(['1', null, 1])(
        ...
        (value) => {
            ...
            expect(() => mutableParser.elseThrow(errorMessgage)).toThrowError(
                message
            );
            expect(() => readonlyParser.elseThrow(message)).toThrowError(
                message
            );
            expect(() => readonlyParser.elseThrow(errorMessgage)).toThrowError(
                message
            );
