...
describe('Object parser', () => {
    it.each([
        ...
        (value) => {
            ...
            const mutableParser = parse(object).asMutableObject(parseObject);
            const readonlyParser = parse(object).asReadonlyObject(parseObject);

            expect(mutableParser.elseThrow(message)).toStrictEqual(object);
            expect(mutableParser.elseThrow(errorMessage)).toStrictEqual(object);
            ...
            expect(mutableParser.elseLazyGet(() => undefined)).toStrictEqual(
                object
            );
            expect(readonlyParser.elseLazyGet(() => undefined)).toStrictEqual(
                object
            );
            expect(
                parseAsMutableObject({
    ...
    it.each(['1', null, 1])(
        ...
        (value) => {
            ...
            expect(() => mutableParser.elseThrow(errorMessage)).toThrowError(
                message
            );
            expect(() => readonlyParser.elseThrow(message)).toThrowError(
                message
            );
            expect(() => readonlyParser.elseThrow(errorMessage)).toThrowError(
                message
            );
            ...
            expect(mutableParser.elseLazyGet(() => undefined)).toBeUndefined();
            expect(readonlyParser.elseLazyGet(() => undefined)).toBeUndefined();
            expect(
                parseAsMutableObject({
                    object,
                    parse: parseObject,
